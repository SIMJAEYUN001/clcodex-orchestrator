...
관리의 기본 진입점은 오케스트레이터 봇의 `/admin`입니다. 명령을 실행한 Administrator에게만 보이는 ephemeral 메시지로 짧은 수명의 링크를 발급하고, 다음 영역을 한 화면에서 관리합니다.
...
`open-codex`의 검색 가능한 model picker, 현재값 우선 표시, approval mode 선택, help/history overlay와 실행 중 model 고정 방식을 웹 UI 상호작용으로 재구성했습니다. `open-codex` 자체를 설치하거나 fork하지 않으며, 공식 Codex와 Claude Code 하네스는 기존처럼 프로젝트 내부에 격리 설치합니다.

Codex 탭에서 역할별로 다음 값을 선택할 수 있습니다.
...
Claude Code 탭에서는 다음 값을 선택할 수 있습니다.
...
설정 우선순위는 현재 포럼 스레드 override → 서버 전체 기본값 → 내장 안전 기본값입니다. 실행 중 하네스 process의 provider/model/runtime policy는 바꾸지 않고 신규 session부터 반영합니다. 자세한 내용은 [Control Center](docs/control-center.md)를 참고합니다.

## 5. 프록시 공급자 설정

Discord native modal의 text input은 password 마스킹을 제공하지 않으므로, `/providers panel`은 **짧은 수명의 관리자 설정 URL**을 발급합니다. URL의 token은 fragment에만 포함되어 HTTP request log로 전송되지 않으며, 서버는 token 원문 대신 SHA-256 digest만 session map에 보관합니다.
...
직접 입력 credential은 AES-256-GCM으로 암호화됩니다. 원문은 Discord 메시지, audit log, HTML 응답 또는 CLI config에 기록하지 않습니다.

### 원격 관리자용 HTTPS

설정 서버는 loopback에만 bind합니다. 다른 장치에서 폼을 열려면 HTTPS reverse proxy를 사용합니다.

```dotenv
ADMIN_SETUP_PUBLIC_URL=https://clcodex-admin.example.com
ADMIN_SETUP_HOST=127.0.0.1
ADMIN_SETUP_PORT=8787
ADMIN_SETUP_SESSION_TTL_MS=600000
```

Reverse proxy에서는 해당 경로를 인터넷 전체에 장기 공개하지 말고, VPN·IP allowlist·추가 SSO 등으로 보호하는 구성을 권장합니다.
...
기본 예시는 [.env.example](.env.example)을 사용합니다.
...
# 비워 두면 npm/pnpm/yarn/bun/node --test/pytest/cargo/go 등 기본 prefix 사용
TASK_COMMAND_PREFIXES=[]
SPEC_AUTO_PUSH=false

PROVIDER_GATEWAY_HOST=127.0.0.1
PROVIDER_GATEWAY_PORT=0
PROVIDER_GATEWAY_ROUTE_TTL_MS=86400000
```

`PROXY_ALLOWED_HOSTS`에는 사설 네트워크 proxy hostname을 명시적으로 등록해야 합니다. loopback 이외의 평문 HTTP endpoint는 거부됩니다.
...
- `/admin` 통합 Control Center와 관리자 권한/application command 소유권
